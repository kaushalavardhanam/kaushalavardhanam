...
| Claude Sonnet 4.6 | `anthropic.claude-sonnet-4-6` / `us.anthropic.claude-sonnet-4-6` / `global.anthropic.claude-sonnet-4-6` | [model card](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-anthropic-claude-sonnet-4-6.html); geo from `us-west-2` | Strong | Unknown vs Sonnet 4 until Mode B | Images | Yes | Yes | 200k class | Newer SKU | None | **Watch-list.** Not selected solely for being newer. This worker: Converse **AccessDenied** (same as Sonnet 4). |
| Claude Haiku 4.5 | `anthropic.claude-haiku-4-5-20251001-v1:0` / `us.anthropic.claude-haiku-4-5-20251001-v1:0` | Profile IDs vary by Region | Strong | Likely good, less reasoned | Images | Yes | Yes | 200k class | Mid | None | Alternate if Sonnet latency/cost is too high. |
...
**This worker’s IAM** (`cursor_worker-execution-role` in `us-west-2`) can call STS and reach `bedrock-runtime` endpoints, but is **denied `bedrock:Converse` / `bedrock:InvokeModel` / `bedrock:ListFoundationModels`**. Probe: `python scripts/eval_bedrock_probe.py` → `evals/results/bedrock_probe.json`. Live shortlist scores must be run on an identity with model access.
